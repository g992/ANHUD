...
class NavigationReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (action) {
            ...
            ACTION_HEADUNIT_NAVIGATION_UPDATE -> {
                if (!OverlayPrefs.headunitNavEnabled(context)) {
                    return
                }
                try {
                    ...
                    Log.d(
                        TAG,
                        "Headunit nav: distance=$distanceMeters m time=$timeSeconds s road=\"$road\" eventType=$nextEventType side=$turnSide action=\"$actionText\""
                    )
                    UiLogStore.append(
                        LogCategory.NAVIGATION,
    ...
    companion object {
        ...
        private fun touchNavigatorIntentTimeout(context: Context, action: String) {
            if (action == ACTION_HEADUNIT_NAVIGATION_UPDATE && !OverlayPrefs.headunitNavEnabled(context)) {
                return
            }
