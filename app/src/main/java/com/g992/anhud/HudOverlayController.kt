...
class HudOverlayController(private val context: Context) {
    ...
    private fun applyState(state: NavigationHudState) {
        ...
        val headunitSourceEnabled =
            OverlayPrefs.headunitNavEnabled(context) && state.source == SOURCE_HEADUNIT
        val hideNavigationByDistance = shouldHideManeuverByDistance(state, showPreview)
        ...
        updateManeuver(state.maneuverBitmap, state.maneuverType, previewNav, headunitSourceEnabled)
        updateArrowManeuver(state.maneuverBitmap, state.maneuverType, previewArrow, headunitSourceEnabled)
        if (clock != null) {
            updateClockText()
        }

        val hasManeuverIcon = state.maneuverBitmap != null ||
            (headunitSourceEnabled && resolveManeuverDrawableResId(state.maneuverType) != 0)
    ...
    private fun updateManeuver(
        ...
        when {
            ...
            allowTypeFallback && maneuverType.isNotBlank() -> {
    ...
    private fun updateArrowManeuver(
        bitmap: android.graphics.Bitmap?,
        maneuverType: String,
        preview: Boolean,
        allowTypeFallback: Boolean
