...
class SensorDataService : Service() {
    ...
    private var turnSignalStubLogged = false
    ...
    private val speedReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (action != ACTION_SENSOR_FLOAT_CHANGED && action != ACTION_SENSOR_FLOAT_RESULT) return
    ...
    override fun onCreate() {
        ...
        val speedFilter = IntentFilter().apply {
            addAction(ACTION_SENSOR_FLOAT_CHANGED)
            addAction(ACTION_SENSOR_FLOAT_RESULT)
        }
        ContextCompat.registerReceiver(this, speedReceiver, speedFilter, ContextCompat.RECEIVER_EXPORTED)
        staleSpeedHandler.postDelayed(staleSpeedRunnable, GPS_STALE_CHECK_INTERVAL_MS)
        initTurnSignalIntegrationStub()
    }
    ...
    private fun initTurnSignalIntegrationStub() {
        clearTurnSignalState()
        if (!turnSignalStubLogged) {
            turnSignalStubLogged = true
            UiLogStore.append(
                LogCategory.SENSORS,
                "Поворотники: прослушивание отключено, используются заглушки"
            )
        }
    }
